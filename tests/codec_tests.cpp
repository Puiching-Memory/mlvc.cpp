...
{
    auto model_dir = assets / "models/mlvc-psnr-v1/canonical/640x368";
    if (!std::filesystem::exists(model_dir / "metadata.json"))
        model_dir = assets / "models/mlvc-psnr-v1/onnx-generic/640x368";
...
{
    auto model_dir = assets / "models/mlvc-s-psnr-v1/canonical/640x368";
    if (!std::filesystem::exists(model_dir / "metadata.json"))
        model_dir = assets / "models/mlvc-s-psnr-v1/onnx-generic/640x368";
    if (!std::filesystem::exists(model_dir / "metadata.json"))
        return;
    const mlvc::ModelConfig config = mlvc::load_model_config(model_dir);
